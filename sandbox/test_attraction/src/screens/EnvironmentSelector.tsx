...
export const EnvironmentSelector: React.FC<EnvironmentSelectorProps> = ({
  ...
  const handleStart = () => {
    if (selectedEnvironment) onSelectEnvironment(selectedEnvironment);
  };

  return (
    <div style={styles.container}>
      <TopBar title="Choose Your Paradise" onBack={onBack} showStats={false} />
          ...
          <div style={styles.grid}>
            {ENVIRONMENTS.map((env) => (
              ...
              >
                ...
                {selectedEnvironment === env.type && <div style={styles.checkmark}>✓</div>}
              </button>
            ))}
          </div>
        </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  cardIcon: { fontSize: 'clamp(2rem, 8vw, 2.5rem)', flexShrink: 0 },
  cardContent: { flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 },
  cardName: { fontSize: 'clamp(0.9rem, 3.5vw, 1rem)', fontWeight: 'bold', color: '#fff' },
  cardDescription: { fontSize: 'clamp(0.7rem, 2.5vw, 0.75rem)', color: 'rgba(255, 255, 255, 0.6)', lineHeight: '1.2' },
  checkmark: {
    width: '20px',
    height: '20px',
    display: 'flex',
