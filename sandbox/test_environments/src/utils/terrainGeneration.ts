...
export function getDistanceFromBottomLeft(
  ...
  // Distance from bottom-left corner (row = mapRows-1, col = 0)
  const bottomRow = mapRows - 1;
  const leftCol = 0;
  
  // In isometric view, we measure distance perpendicular to the edge
  // For bottom-left edge, we measure distance diagonally
...
// Simple 2D Perlin-like noise (simplified for terrain generation)
export class SimplexNoise {
  ...
  noise2D(x: number, y: number): number {
    // Very simple pseudo-random noise based on position
